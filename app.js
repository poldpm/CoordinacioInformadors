...
function prepararColumnes() {
  columnes = dades.capçaleres.map((nom, index) => {
    return { nom: nom, index: index, tipus: detectarTipus(index) };
  });
}

// Detecta si una columna és numèrica, de data o de text
function detectarTipus(index) {
  let nNum = 0, nData = 0, nTotal = 0;
  for (let i = 0; i < dades.files.length && i < 50; i++) {
    const v = dades.files[i][index];
    if (v === '' || v === null || v === undefined) continue;
    nTotal++;
    if (esData(v)) nData++;
    else if (!isNaN(Number(v))) nNum++;
  }
  if (nTotal === 0) return 'text';
  if (nData / nTotal > 0.6) return 'data';
  if (nNum / nTotal > 0.6) return 'number';
  return 'text';
}
...
function omplirSelectors() {
  ...
  columnes.filter(c => c.tipus === 'text' || c.tipus === 'data').forEach(c => {
    const o = document.createElement('option');
  ...
  return dades.files.filter(fila => {
    return filtres.every(f => {
      ...
      if (col && col.tipus === 'number') {
        const a = Number(cel), b = Number(f.valor);
