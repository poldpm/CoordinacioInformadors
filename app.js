...
function construirOpcionsMetriques() {
  const opcions = [];
  opcions.push({ id: '__count__', nom: tipusActual === 'gossos' ? 'Total de gossos' : 'Nombre de registres' });
  ...
  // Itinerància: total de cada columna numèrica (sense "Suma de" ni "Mitjana de",
  // que confonen; el total és el que volen). La mitjana la deixem com a opció apart.
...
// Opcions d'agrupament en llenguatge humà, només les que tenen sentit.
function construirOpcionsAgrupar() {
  ...
  // Per zona / àrea
  if (cZona) ops.push({ nom: tipusActual === 'gossos' ? 'Per zona' : 'Per àrea', valor: String(cZona.index) });

  // Gossos: per resposta i per estat
  const cEstat = columnes.find(c => c.nom.toLowerCase().trim() === 'estat');
  const cResp = columnes.find(c => c.nom.toLowerCase().trim() === 'resposta');
  if (cResp) ops.push({ nom: 'Per resposta', valor: String(cResp.index) });
  if (cEstat) ops.push({ nom: 'Per estat (lligat/deslligat)', valor: String(cEstat.index) });

  // Itinerància: per tipus (tram/aparcament) i per punt
  const cTipus = columnes.find(c => c.nom.toLowerCase().trim() === 'tipus');
  const cPunt = columnes.find(c => c.nom.toLowerCase().trim() === 'punt');
  if (cTipus) ops.push({ nom: 'Per tipus (tram/aparcament)', valor: String(cTipus.index) });
  if (cPunt) ops.push({ nom: 'Per punt', valor: String(cPunt.index) });

  // Evolució temporal (només les granularitats útils)
  if (cData) {
...
function muntarFiltresRapids() {
  ...
  // Quines columnes val la pena filtrar? Les de text amb pocs valors únics,
  // EXCEPTE les que ja són mètrica (Estat/Resposta a gossos) per no duplicar.
  const jaSonMetrica = (tipusActual === 'gossos')
    ? ['estat', 'resposta']
    : [];
  ...
  const colsFiltrables = columnes.filter(c => {
    ...
    if (['id', 'núm', 'num', 'observacions', 'punt'].indexOf(n) !== -1) return false;
    if (jaSonMetrica.indexOf(n) !== -1) return false;
    const vals = valorsUnics(c.index).filter(v => v !== '' && v !== '(buit)');
    return vals.length >= 2 && vals.length <= 15; // pocs valors -> caselles
  });

  if (colsFiltrables.length === 0) {
    cont.innerHTML = '<p class="bloc-ajuda" style="margin:0;">No hi ha res a filtrar en aquestes dades.</p>';
    document.getElementById('blocFiltres').style.display = colsFiltrables.length ? '' : 'none';
    return;
  }
...
function aplicarFiltres() {
  ...
  if (grupsActius.length === 0) return dades.files.slice();
  return dades.files.filter(fila =>
    grupsActius.every(g => g.valorsSel.has(String(fila[g.colIndex])))
  );
}
...
// Etiqueta de setmana curta i ordenable: "2026-S25"
function setmanaEtiqueta(v) {
  ...
  return dt.getUTCFullYear() + '-S' + (setmana < 10 ? '0' + setmana : setmana);
}
