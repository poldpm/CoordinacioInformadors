...
function muntarMetriques() {
  ...
  const opcions = [];
  // Sempre: nombre de registres
  opcions.push({ id: '__count__', nom: 'Nombre de registres' });
  // Percentatge sobre el total (útil sobretot per a gossos)
  opcions.push({ id: '__pct__', nom: '% sobre el total' });
  // Sumes i mitjanes de columnes numèriques
  ...
  // Per defecte, seleccionem "Nombre de registres"
  const primer = cont.querySelector('.chip-metrica');
  if (primer) { primer.classList.add('sel'); metriquesSel.push('__count__'); }
}
...
function nomMetrica(m) {
  if (m === '__count__') return 'Nombre de registres';
