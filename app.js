...
function generarInforme() {
  ...
  let series, claus, agrupar, subtitolExtra = '';

  if (puntsActius && metriquesSel.length === 1) {
    // MODE PUNTS: eix X = temps (dia/setmana/mes), una barra per cada punt.
    // Cada "sèrie" és un punt; es reparteix per l'agrupació temporal escollida.
    agrupar = agruparSel;
    const metrica = metriquesSel[0];
    const puntsList = Array.from(filtrePunts.valorsSel).sort();

    series = puntsList.map(punt => {
      const filesPunt = files.filter(f => String(f[filtrePunts.colIndex]) === punt);
      return {
        id: 'punt:' + punt,
        nom: punt,
        dades: agregar(filesPunt, metrica, agrupar, filesPunt.length)
      };
    });
    claus = unificarClausOrdenat(series);
    subtitolExtra = ' · ' + nomMetrica(metrica);
  } else if (puntsActius && metriquesSel.length > 1) {
    // Amb punts triats però diverses mètriques, avisem i agrupem per punt simple.
    toast('Per veure els punts al llarg del temps, deixa només una dada a “Què vols veure”.');
    agrupar = String(filtrePunts.colIndex);
    series = metriquesSel.map(m => ({ id: m, nom: nomMetrica(m), dades: agregar(files, m, agrupar, files.length) }));
    claus = unificarClaus(series);
  } else {
    // MODE NORMAL: una sèrie per mètrica, repartides pel temps o filtre triat.
    agrupar = agruparSel;
    series = metriquesSel.map(m => ({ id: m, nom: nomMetrica(m), dades: agregar(files, m, agrupar, files.length) }));
    claus = unificarClaus(series);
  }

  renderitzarPissarra(files, series, claus, agrupar, subtitolExtra);
...
function renderitzarPissarra(files, series, claus, agrupar, subtitolExtra) {
  ...
  const modePunts = (filtrePunts && filtrePunts.actiu && filtrePunts.valorsSel.size > 0 && metriquesSel.length === 1);
  ...
  // Títol i subtítol del gràfic
  let titol, subtitol;
  if (modePunts) {
    titol = 'Punts triats' + (subtitolExtra || '');
    subtitol = 'per ' + nomAgrupar(agrupar) + ', comparant ' + series.length + ' punts · ' + files.length + ' registres';
  } else {
    titol = series.map(s => s.nom).join(' · ');
    subtitol = 'per ' + nomAgrupar(agrupar) + ' · ' + files.length + ' registres';
  }
...
function crearGrafic(series, claus, agrupar) {
  ...
  // Plugin per escriure el nom de cada punt en vertical, dins de la barra,
  // quan estem en mode punts amb barres (com demana l'usuari).
  const etiquetaBarresPlugin = {
    id: 'etiquetaBarres',
    afterDatasetsDraw(chart) {
      if (!modePunts || vistaActual !== 'barres') return;
      const { ctx } = chart;
      chart.data.datasets.forEach((dataset, di) => {
        const meta = chart.getDatasetMeta(di);
        meta.data.forEach((barra, bi) => {
          const valor = dataset.data[bi];
          if (!valor) return;
          const nom = dataset.label;
          ctx.save();
          ctx.translate(barra.x, barra.y + 6);
          ctx.rotate(-Math.PI / 2);
          ctx.fillStyle = '#ffffff';
          ctx.font = '600 11px Inter, sans-serif';
          ctx.textAlign = 'left';
          ctx.textBaseline = 'middle';
          // retallem el nom si és massa llarg per l'alçada de la barra
          const alcada = chart.scales.y.bottom - barra.y - 10;
          let text = nom;
          if (ctx.measureText(text).width > alcada && alcada > 20) {
            while (ctx.measureText(text + '…').width > alcada && text.length > 3) text = text.slice(0, -1);
            text += '…';
          }
          if (alcada > 20) ctx.fillText(text, 0, 0);
          ctx.restore();
        });
      });
    }
