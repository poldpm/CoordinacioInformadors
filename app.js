...
function detectarTipus(index) {
  ...
  for (let i = 0; i < dades.files.length; i++) {
    ...
    nTotal++;
    if (esData(v)) { nData++; continue; }
    if (esNumero(v)) nNum++;
  }
  if (nTotal === 0) return 'text';
  // Prou tolerant: si més de la meitat dels valors no buits són números
  if (nData / nTotal > 0.5) return 'data';
  if (nNum / nTotal > 0.5) return 'number';
  return 'text';
}
